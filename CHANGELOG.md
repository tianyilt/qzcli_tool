...
v0.4.16 做到了「检测到多处密码不一致就把指纹摆出来」，但**摆完照样拿优先级最高的
那个去打认证**。认证服务按失败次数锁账号（5 次），所以那等于拿账号的锁定额度去赌
哪个密码是对的 —— 而且每条命令赌一次。检测到了就必须能挡住，这一版把挡住做了。

- **`qzcli login` 冲突时直接退出，不发请求。** 退出码 1，列出每一处的指纹，
  并给出三条明确出路：`--source env|envfile|config`（只认那一处）、
  `--password-stdin`（当场给）、`--force`（坚持按优先级来）。
  判据收得很紧：只有当**即将发出去的那个密码**确实是有歧义那堆里优先级最高的
  那个时才拦 —— 误报会让人学会一律加 `--force`，那这道闸门就白装了。
- **自动重登同样被挡，而且这条更重要。** `api._relogin` 用的是进程启动时取到的
  密码。用户只敲一句 `qzcli status`，撞上过期 cookie 就**静默**去打一次认证 ——
  全程没人看见。改密码**之前**就 `export` 过 `QZCLI_PASSWORD` 的长期进程
  （编辑器、agent、tmux）于是每条命令都在悄悄扣额度；2026-09-12 实测本机还有
  113 个这样的进程。现在冲突会抛出带指纹表的明确错误，并记进重登冷却，
  避免并发的每个线程各报一遍。
- **`/auth/token` 那条路径也挡。** 它把密码**明文**POST 出去，同样是一次凭据
  验证 —— 错误的凭据在哪个端点上失败都是失败。
- **自动重登失败现在会写操作日志**（`op=relogin`，已加入 `RECORDED_OPS`）。
  它不经过 `main()` 的分发点，此前失败**一个字都不记**：排查时会看到「本机没登录
  过」，而平台侧审计记着一串失败，两边对不上。
- 新增 `config.credential_conflict()`（返回判据，不只是一句提示）、
  `config.get_credentials_from_source()`、`config.password_fingerprint()`；
  `get_credentials()` / `get_credentials_with_source()` 签名与行为不变。
- **测试不再污染用户真实的操作日志，这次是进程级隔离。** 上一版逐个文件打的补丁
  不够用：`_relogin` 一加记录点，四个一行没改的既有测试文件立刻开始漏
  （test_auth_retry / test_auth_before_fanout / test_job_v2 / test_relogin_dedup）。
  更坑的是 `tests/__init__.py` 在文档写的那条命令（`discover -s tests`）下
  **根本不会被导入** —— `-s` 指向目录时 unittest 按顶层模块导入，整个包 init 被跳过，
  实测过。现在隔离放在 `tests/test_opslog_isolation.py` 的 import 期（discover 会先
  导入全部测试模块、再执行任何一条），并留一条运行期判据：测试进程里
  `opslog.log_path()` 不许指向真实状态目录。
- 新增 `tests/test_credential_conflict_blocks_requests.py`（19 条）。每条的核心断言
  都是同一句：**`login_with_cas` 调用次数为 0**。只断言「报了错」不够 ——
  报错之后照样发请求正是这版要修掉的形状。已用变异测试确认：摘掉守卫后这些用例变红。

## v0.4.16 - 2026-09-12

**主题：让「密码明明对却登不上」这件事一眼可见，并且不让日志再说反话。**

起因是 2026-09-11 的一次真实排查：账号反复被 CAS 锁定，用户改了密码、改了
`~/.qzcli/config.json`，`qzcli login` 仍然报「账号或密码错误」。真因是 shell 里一条
**变更前就 export 过的** `QZCLI_PASSWORD` —— 它优先级高于配置文件，静默压过了新密码。
排查中还白白消耗了 CAS 的锁定计数。

- **`login` 现在会明说密码是从哪读的。** 登录前打印
  `凭据来源: 用户名=… | 密码=…`，取值可能是 `环境变量 QZCLI_PASSWORD` /
  `~/.qzcli/.env` / `~/.qzcli/config.json` / `命令行 --password` / `标准输入` / `交互式输入`。
  新增 `config.get_credentials_with_source()`；`get_credentials()` 签名和行为不变。
- **登录失败时，若多处存着不一样的密码，直接把冲突摊开。**
  `config.describe_credential_conflict()` 按优先级列出每一处及其**指纹**
  （sha256 前 8 位，**绝不回显明文**），并点明「优先级更高的那处没更新」
  「环境变量来自 ~/.zshrc，已启动的进程不会自动更新」。
  只有一处、或多处值相同时**保持沉默** —— 误报会让人学会忽略提示。
- **失败的操作不再被记成成功。** `cmd_*` 约定用「返回非 0」表示失败
  （`cmd_login` 捕获异常后 `return 1` 就是典型），而 `opslog.timed` 只看有没有抛异常，
  于是**一次登录失败被写成 `outcome: ok`**。当时本机日志里 152 次登录全是「成功」，
  平台侧审计同期却记着一串「密码错误」，两边对不上，反而把排查带偏。
  现在分发点会在返回码非 0 时调用新增的 `timed.mark_failed()`，记 `outcome: error`
  并带上 `err_class=exit=<码>`。这一处修好，**所有**走 `RECORDED_OPS` 的命令都跟着修好。
- **测试不再往用户真实的操作日志里写假记录。** 排查过程中发现
  `~/.qzcli/qzcli_ops.log` 里混着 `argv=["python3 -m unittest", …]` 和
  `argv=["qzcli","create","-i"]`、`duration_ms=0` 的条目 —— 是两条走真
  `cli.main()` 的用例和一条没隔离的用例写进去的。**这份日志正是给「账号为什么
  反复被锁」取证的地方**，被测试写脏比没记更糟。现在这些用例把
  `QZCLI_OPS_LOG` 重定向到临时目录，并加了一条守门用例：凡是出现
  `cli.main()` 的测试文件，同一文件里必须出现 `QZCLI_OPS_LOG`，否则红。
- 新增 `tests/test_credential_source.py`（13 条），守住：来源必须说得出、
  冲突必须报且不许回显明文、值相同时不许吵、返回非 0 必须记成 error、
  诊断设施不许吞异常、测试不许污染真实审计日志。
...
- **`avail` 的表头改回「GPU利用率」。** v0.4.13 把它改成了「GPU分配率」，
  理由是这个数算的确实是 `(总卡 − 空闲卡) / 总卡`。但**改名的动机是一个次要场景**
  （判断机器会不会被空闲回收），而这一列的日常用途是「还能不能起任务」——
  在这个用途上两个口径给出的判断一样。为一个用不到的场景去动天天在看的表头，
  收益不抵改动成本。
  - **数字从头到尾没变过**，v0.4.13 那次也只动了标签和内部变量名
  - 内部变量保持 `gpu_alloc_ratio`（按实际含义命名），算式旁的注释写清了
    「表头显示利用率是刻意的，不是没改完」
  - 测试改成守真正该守的：算式口径不被换掉、真实利用率那条来源
    （`gpu.usage_rate`，`dashboard` 走它）不被误删、区别的说明必须留在算式旁边
  - 唯一要记住的是：**别拿 `avail` 这一列去判断"会不会被空闲回收"** ——
    平台那个判据看真实利用率，跟分配率结论相反

## v0.4.13 - 2026-08-27

**主题：四处「看起来能跑、结论却是反的」。**

对照 InspireSkill 手册做的一轮补齐，外加闸门跑出来的一个真 bug。这四条都不是
「不能跑」，而是**跑得好好的、答案是错的**：

- `qzcli logs` **对所有任务都是坏的**，而报错文案指向完全无关的方向
- `avail` 的列名把**分配率**叫成利用率，在「我这台会不会被回收」上给出相反结论
- 优先级被规格拒绝时，报错原样抛给用户，他不知道该改什么
- 新加的节点诊断如果照直觉写，会把**健康的机器全报成有病**

`qzcli logs` 那条不是本轮引入的 —— 在 v0.4.12 的 worktree 上跑同一段探针一模一样地挂，
是命名约定漂了之后一直没对过。

### 新增

- **`qzcli events --node <节点名>`：查机器自己的健康。** `events <job>` 回答的是
  「我这个任务为什么没起来」，答不了另一个问题：**是不是这台机器本身有病**。
  qzcli 早就有 `--exclude-node`，但此前**没有任何东西告诉你该排除谁**。
  数据源是 `cluster ListNodeEvents` —— 平台上唯一按节点而非按工作负载组织的事件源，
  给的是机器病历：`XIDIsUnhealthy`（GPU 硬件错误）/ `GPFSMountUnhealthy`
  （共享盘挂载坏，**日志里看起来像你的代码卡住了，其实是盘**）/ `NodeNotReady` 等。
  输出分「现在有问题」和「曾经出过、已恢复」两栏，并给出能直接粘到 `create` 后面的
  排除串。开发中真机打出来的四个坑（每个都会把结论搞反）：
  - 过滤条件在 `filter.node_names` **嵌套数组**里，平铺传 `node_name` 会被拒
  - **一次问多台会被总条数分页挤掉后面的**：问 5 台只覆盖前 2 台，后 3 台被诊断成
    「没发现异常记录」—— 静默假阴性。改成逐台查
  - **平台按时间升序返回，第 1 页是最旧的**：`total=222` / `page_size=200` 时丢掉最新
    22 条，拿 1 月的数据判断"现在"。改成取最后一页
  - **`Healthy` 是 `Unhealthy` 的子串**：`XID.*Healthy` 忽略大小写时也匹配
    `XIDIsUnhealthy`，把正在报错的机器读成"已恢复"。改用 `(?<!un)healthy`
  - 另外：**陈旧的「未恢复」不算现存故障**。平台只在状态翻转时记事件、不保证记恢复，
    实测一台正常在跑任务的机器最后一条 GPFS 事件是 108 天前的 Unhealthy —— 按"最后
    一次翻转"判会建议把它排除掉，照这个报警半个机房都得排。超过 7 天的降级成如实
    陈述，不建议排除
...
- **`qzcli logs` 对所有任务都是坏的 —— 因为在猜平台的实例命名约定。**
  取日志前要先给出实例名，qzcli 一直按 `{job_id}-worker-{i}` 拼；而平台真实的实例名是
  `{job_id}-worker-{i}-{round}`，**末尾多一段**。少那一段，日志接口回
  `InvalidParameter: Invalid instance names, the job ids length of instances except 1,
  but got 0` —— 报错文案跟真实原因（名字拼错了）毫无关系，所以一直没人往这上面想。
  - 实测扫 60 个任务 **0 条日志**；改成问平台要真名（`train ListJobInstances`）后
    **5/5 全部取到**
  - v0.4.12 一样挂，**不是新回归**，是约定漂了之后一直没对过
